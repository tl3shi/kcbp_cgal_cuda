...
int main(int argc, char** argv)
{
    ...
    if(argc >= 2) 
        k = atoi(argv[1]);
    if(k > 46){
        cout << "KDOP k max is 46" << endl;
        k = 46;
    }
    if((k & 0x1) == 1){
        cout << "KDOP k should not be odd" << endl;
        k = k-1;
    }
    cout << "K = " << k << endl;
    if(argc >= 6)
   ...
   //KDop3D::projectCPU(points3d, normals, planes, benchtest);
    ...
    auto kdopNormals = KCBP::getCollDetNormals(k);
    printf("kdop normalsize = %d\n", kdopNormals.size());
    vector<Plane3D> kdopplanes;
    SSEProjection::projectCPUSSE(points3d, kdopNormals, kdopplanes, 1);
    KDop3D::getResultByDualMapping(kdopplanes, kdoppolyhedra);
    printf("kdop Polytopes %d\n", kdoppolyhedra.size());
    double kdopvolume = KCBP::getVolume(kdoppolyhedra);
    printf("KDOP Volume: %.12f\n", volume);
    ...
    if (draw_convexhull)
    {
        ...
        cout << "KCBP tightness: " << convexvolume / volume << endl;
        cout << "KDOP tightness: " << convexvolume / kdopvolume << endl;
        cout << "delta:" << convexvolume / volume  - convexvolume /kdopvolume << endl;
    }
