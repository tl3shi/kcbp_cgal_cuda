...
struct kcbpNode
{
    vector<Polygon3D*> node;
    vector<CP_Vector3D> data;
    kcbpNode(vector<Polygon3D*> &n, vector<CP_Vector3D> &d):node(n), data(d){}
};


vector<vector<kcbpNode*>> kcbpBVH;
...
kcbpNode *  AABBNode2kCBP(AABBNode * node, int k)
{
    vector<CP_Vector3D> points3dt;
    for(int i = 0; i < node->DataSize; i++)
    {
        PrimitivePtr p = node->Data[i];
        points3dt.push_back(p->v0);
        points3dt.push_back(p->v1);
        points3dt.push_back(p->v2);
    }
    vector<CP_Vector3D> acht =  ACH3D::getACH(points3dt, 10, 10);
    vector<CP_Vector3D> normalst = KCBP::getClusterNormals(acht, k, true, false);
    vector<Plane3D> planest;
    SSEProjection::projectCPUSSE(points3dt, normalst, planest, 1);
    vector<Polygon3D*> kcbp;
    //KDop3D::getResultByDualMapping(planest, kcbp);
    KDop3D::getResultByThreePlane(planest, kcbp);
    return new kcbpNode(kcbp, points3dt);
}
...
int main(int argc, char** argv)
{
    ...
    while(!q.empty())
    {
        ...
        while(q.front() != NULL)
        {
            ...
            kcbpNode * knode = AABBNode2kCBP(top, k);
            layerKCBP.push_back(knode);
        }
        q.pop();
        bvh.push_back(layer);
        kcbpBVH.push_back(layerKCBP);
    }
...
void  display(void)
{
    ...
    if(true)
    {
        ...
        if(false)
        {
            UIHelper::drawPolygons3D(polyhedra, 0, polyhedra.size(), polygon_mode, 1.0);
        ...
        {
            glColor3f(0.0, 0.0, 1.0);
            if(bvh_layer >= kcbpBVH.size()) bvh_layer = 0;
            vector<kcbpNode *> node =  kcbpBVH[bvh_layer];
            for (int i = 0;  i < node.size(); i++)
            //int i = bvh_layer == 0 ? 0 : 1;
            {
                UIHelper::drawPolygons3D(node[i]->node, 0, node[i]->node.size(), polygon_mode, 1.0);
                //UIHelper::drawFacets(node[i]->data);
            }
        }
...
void gl2ps()
{
    ...
    string epsfilename = objfilename + "-kcbp-bvh" + to_string(bvh_layer) +".pdf";
