...
void supportConvex2(const void* obj, const ccd_vec3_t* dir_, ccd_vec3_t* v)
{
    ...
    */    
       ...
       */
       ...
       {
            float xyzw[4];
            Rotmat2Quar(c->transformMatrix, xyzw[0], xyzw[1], xyzw[2], xyzw[3]);
            ccd_quat_t src_;
            ccd_quat_t dest;
            ccdQuatSet(&src_, xyzw[0], xyzw[1], xyzw[2], xyzw[3]);
            ccdQuatInvert2(&dest, &src_);
            mat4 tmpM = Quar2Rotmat(dest.q[0], dest.q[1], dest.q[2], dest.q[3]); //this matrix'3X3 shoule be same as imatrix up(attemp1) ===> same
            ccd_vec3_t newDirCCD;
            ccdVec3Copy(&newDirCCD,  dir_);
            ccdQuatRotVec(&newDirCCD, &dest);
            int max_index = 0;
            maxdot = - RealValueTypeMax;
            CP_Vector3D newDir(newDirCCD.v[0], newDirCCD.v[1], newDirCCD.v[2]);//same as ? newDir up(attemp1)  ===¡· no
            for(int i = 0; i < c->points.size(); i++)
            {
                RealValueType dot = c->points[i] * newDir;
                if(dot > maxdot)
            ...
            CP_Vector3D newP = c->transformMatrix * c->points[max_index];
            ccdVec3Set(v, newP.x, newP.y, newP.z);
       }
}
