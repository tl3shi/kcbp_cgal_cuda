...
vector<CP_Vector3D> initialnormals;
vector<Polygon3D*> polyhedra;
vector<Polygon3D*> initialpolyhedra;
...
int main(int argc, char** argv)
{
    ...
    initialnormals = KCBP::genNormalsEqualArea(k);
    ...
    vector<Plane3D> initialPlanes;
    SSEProjection::projectCPUSSE(points3d, initialnormals, initialPlanes, 1);
    ...
    KDop3D::getResultByDualMapping(initialPlanes, initialpolyhedra);
    ...
    if(false)
    {
        ...
        double initial_volume = KCBP::getVolume(initialpolyhedra);

        cout << "kmeans tightness: " << convexvolume / kmeans_volume << endl;
        cout << "initial tightness: " << convexvolume / initial_volume << endl;
        cout << "delta:" << convexvolume / kmeans_volume - convexvolume / initial_volume  << endl;
        //k=35 kmeans: 0.915485   inital: 0.795681  delta = 0.119804
        //k=26 kmeans: 0.892616   inital: 0.742734  delta = 0.14988
    }
...
void  display1(void)
{
   ...
   
    {
        ...
        //drawAxes(CP_Vector3D(0,0,0), 10.0);
...

        {
            ...
            UIHelper::drawPolygons3D(initialpolyhedra, 0, initialpolyhedra.size(), polygon_mode, draw_scale);
            ...
            gluSphere(qobj,Ball_radius * draw_scale,60,60);
            glLineWidth(2.0);  
            setMatirial(normalcolor);      
            UIHelper::drawNormalsWithArrow(initialnormals, Ball_radius * draw_scale);
            glPointSize(5.0f);
            setMatirial(pointcolor);
            UIHelper::drawPoints(initialnormals, Ball_radius * draw_scale);
        }
        ...
        
        {
            ...
            setMatirial(normalcolor);
            UIHelper::drawNormalsWithArrow(normals, Ball_radius * draw_scale);
            glPointSize(5.0f);
            setMatirial(pointcolor);
