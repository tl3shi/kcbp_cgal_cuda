...
below %2.2g of the nearest %sfacet.\n",
  ...
  FOREACHfacet_i_(facets) {  /* for each point with facet assignment */
	...
	waserror= True;
	fprintf(qh fout, "qhull precision error: point p%d is outside facet f%d, distance= %6.8g maxoutside= %6.8g\n", 
  ...
  if (notverified && !qh DELAUNAY && !qh_QUICKhelp && qh PRINTprecision) 
    fprintf(qh fout, "%d points were well inside the hull.  If the hull contains\n\
...
void qh_check_points (void) {
  ...
  if (qh RANDOMdist) /* repeated computations can differ by 2*distround */
    maxoutside += qh DISTround;
  trace1((qh fout, "qh_check_points: check all points below %2.2g of all facet planes\n",
	  maxoutside));
